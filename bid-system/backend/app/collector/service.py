...
_TITLE_INDUSTRY_KEYWORDS: list[tuple[list[str], str]] = [
    # ── 도장ㆍ습식ㆍ방수ㆍ석공 ──────────────────────────────────────────────
    (["방수공사", "방수처리", "방수 공사", "방수도장", "옥상방수", "지붕방수",
      "외벽방수", "방수보수", "드라이비트", "드라이 비트",
      "석면철거", "석면해체"],                                    "도장ㆍ습식ㆍ방수ㆍ석공사업"),
    (["도장공사", "도장 공사", "페인트", "도색공사", "외벽도장"],  "도장ㆍ습식ㆍ방수ㆍ석공사업"),
    (["미장공사", "타일공사", "석공사", "습식공사", "화장실 보수", "화장실보수",
      "화장실 수선", "화장실수선"],                                "도장ㆍ습식ㆍ방수ㆍ석공사업"),
    # ── 실내건축 ────────────────────────────────────────────────────────────
    (["실내건축공사", "인테리어공사", "인테리어 공사", "내장공사", "칸막이공사",
      "목공사", "도배공사", "공간혁신", "학교공간혁신", "공간 혁신",
      "승강기교체", "엘리베이터교체", "승강기 교체", "엘리베이터 교체",
      "바닥교체", "바닥 교체", "바닥재", "환경개선공사", "환경 개선",
      "실습동", "생활관", "강당 보수", "강당보수", "회의실 보수",
      "사무실 보수", "사무실보수", "기숙사", "세탁방", "체력단련실",
      "리모델링", "내부개선", "내부 개선", "내부 환경"],          "실내건축공사업"),
    # ── 금속창호ㆍ지붕건축물 ────────────────────────────────────────────────
    (["창호공사", "창문공사", "유리공사", "지붕공사", "금속공사",
      "방화셔터", "방화문교체", "방화문 교체", "셔터교체", "셔터 교체",
      "창호교체", "창호 교체", "외벽교체", "외벽 교체",
      "지붕개량", "지붕 개량", "슬레이트", "차양시설", "차양 시설"],
                                                                   "금속창호ㆍ지붕건축물조립공사업"),
    # ── 이하 비활성 업종 (키워드 순서로 우선 배제용) ──────────────────────
...
def sync_inpo21c_notices_to_bids(db: Session) -> dict:
    ...
    try:
        # CockroachDB 시퀀스 desync 방지: INSERT 전 시퀀스를 MAX(id)+1로 리셋
        for _tbl in ("agencies", "bids"):
            try:
                db.execute(text(
