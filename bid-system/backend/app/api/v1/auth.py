...
def login(body: LoginRequest, db: Session = Depends(get_db)):
    ...
    return TokenResponse(access_token=token, user_name=user.name or user.username, role=user.role)
...
def refresh(current: User = Depends(get_current_user)):
    ...
    return TokenResponse(access_token=token, user_name=current.name or current.username, role=current.role)
